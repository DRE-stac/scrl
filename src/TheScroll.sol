...
function _beforeTokenTransfer(address from, address to, uint256 tokenId) 
    ...
    whenNotPaused // if you want to include the Pausable functionality
{
    super._beforeTokenTransfer(from, to, tokenId);

    // Remove the token from the list of the previous owner
    if (from != address(0)) {
        _removeTokenFromOwnerEnumeration(from, tokenId);
    }

    // Add the token to the list of the new owner
    if (to != address(0)) {
    ...
    // Implement the _removeTokenFromOwnerEnumeration function
    function _removeTokenFromOwnerEnumeration(address from, uint256 tokenId) private {
        // This is a simplified version. You'll need to handle array manipulation
        // in a way that removes the correct tokenId.
